...
- ⚡ **地址池**：从公开地址池读取候选入口，统一使用当前 Pages 后端与连接参数生成节点。
...
本分支固定使用一个 Cloudflare Pages 项目，不创建第二套 Worker、Pages 或账号。
订阅节点的连接参数继续由当前 Pages 配置生成，公开地址池只提供入口 IPv4、端口
和地区备注。

- 唯一来源：
  `LancelotRar/best-cf-ips/main/best-cf-ipv4.txt`
- 更新间隔：3 小时
- 更新规则：忽略说明行和空行，严格校验公网 IPv4 与端口，按 `IP+端口` 去重，
  不做 ASN、Cloudflare 网段、服务端延迟、吞吐或 Top-N 筛选
- 安全更新：每次把完整结果写入一个 KV 快照；上游失败、结果为空或新数量不足
  当前数量的 30% 时，继续使用最后一次成功快照
- 节点命名：`[池]地区-序号`，顺序稳定，不附带第三方订阅品牌
- 输出：沿用 edgetunnel 的 Base64、Mihomo/Clash、Shadowrocket 等订阅格式；
  本改造不修改原有分流规则

运行状态可通过 `/.well-known/address-pool/status` 查看。定时工作流只向当前生产
Pages 的刷新入口发出请求，不保存订阅凭据或连接参数。
